...
amiid = sys.argv[3] if len(sys.argv) > 3 else ''
instance_type =sys.argv[3] if len(sys.argv) > 3 else ''
stage = sys.argv[5] if len(sys.argv) > 5 else 'dev'
...
cloudformation.deploy_stack(stacks.jenkins(tenant=tenant,vpc=vpc,subnet=subnet,amiid=amiid,instance_type=instance_type))
